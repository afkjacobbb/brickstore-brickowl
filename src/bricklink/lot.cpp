...
Lot &Lot::operator=(const Lot &copy)
{
    ...
    m_brickOwlLotId        = copy.m_brickOwlLotId;
    m_brickOwlId           = copy.m_brickOwlId;
    m_brickOwlColorId      = copy.m_brickOwlColorId;
    m_brickOwlCondition    = copy.m_brickOwlCondition;
...
bool Lot::operator==(const Lot &cmp) const
{
    return (!m_incomplete && !cmp.m_incomplete)
            ...
            && (m_brickOwlLotId     == cmp.m_brickOwlLotId)
            && (m_brickOwlId        == cmp.m_brickOwlId)
            && (m_brickOwlColorId   == cmp.m_brickOwlColorId)
            && (m_brickOwlCondition == cmp.m_brickOwlCondition)
...
void Lot::save(QDataStream &ds) const
{
    ds << QByteArray("II") << qint32(6)
       ...
       << m_brickOwlLotId << m_brickOwlId << m_brickOwlColorId << m_brickOwlCondition;
}
...
Lot *Lot::restore(QDataStream &ds, uint startChangelogAt)
{
    ...
    if (version >= 6)
        ds >> lot->m_brickOwlLotId >> lot->m_brickOwlId
           >> lot->m_brickOwlColorId >> lot->m_brickOwlCondition;
