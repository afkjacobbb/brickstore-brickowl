    ...
    : QDialog(parent)
{
    ...
    auto *boPage = new QWidget(this);
    boPage->setObjectName(u"brickowl"_qs);
    auto *boLayout = new QVBoxLayout(boPage);
    auto *boGroup = new QGroupBox(tr("BrickOwl account"), boPage);
    auto *boForm = new QFormLayout(boGroup);
    m_brickOwlKey = new QLineEdit(boGroup);
    m_brickOwlKey->setEchoMode(QLineEdit::Password);
    boForm->addRow(tr("API key"), m_brickOwlKey);
    auto *boTest = new QPushButton(tr("Test connection"), boGroup);
    auto *boStatus = new QLabel(boGroup);
    boStatus->setWordWrap(true);
    boForm->addRow(boTest, boStatus);
    boLayout->addWidget(boGroup);
    auto *help = new QLabel(tr("Create an API key with inventory read/write and catalogue permissions in your BrickOwl account. Credentials are saved using BrickStore's credential manager."), boPage);
    help->setWordWrap(true);
    boLayout->addWidget(help);
    boLayout->addStretch();
    w_tabs->insertWidget(3, boPage);
    auto *boTab = new QTreeWidgetItem;
    boTab->setText(0, tr("BrickOwl"));
    w_tab_list->insertTopLevelItem(3, boTab);
    connect(boTest, &QPushButton::clicked, this, [this, boTest, boStatus]() -> QCoro::Task<> {
        m_apiTesting = true;
        boTest->setEnabled(false);
        boStatus->setText(tr("Connecting…"));
        // Dialog remains alive while its coroutine is suspended.
        setEnabled(false);
        BrickOwl::Client client(m_brickOwlKey->text().trimmed());
        try {
            const auto inventory = co_await client.inventory();
            boStatus->setText(tr("Connected: %1 inventory lots.").arg(inventory.size()));
        } catch (const Exception &e) {
            boStatus->setText(e.errorString());
        }
        setEnabled(true);
        m_apiTesting = false;
        boTest->setEnabled(true);
    });

    auto *apiGroup = new QGroupBox(tr("BrickLink Store API — manual synchronization"), bricklink);
    auto *apiForm = new QFormLayout(apiGroup);
    const QStringList apiLabels { tr("Consumer key"), tr("Consumer secret"), tr("Token"), tr("Token secret") };
    for (size_t i = 0; i < m_storeApiFields.size(); ++i) {
        m_storeApiFields[i] = new QLineEdit(apiGroup);
        m_storeApiFields[i]->setEchoMode(QLineEdit::Password);
        apiForm->addRow(apiLabels.at(qsizetype(i)), m_storeApiFields[i]);
    }
    auto *apiHelp = new QLabel(tr("These four OAuth credentials are separate from the BrickStore access token above. Register your current public IP in BrickLink's Store API settings. Existing BrickLink import/export remains unchanged."), apiGroup);
    apiHelp->setWordWrap(true);
    apiForm->addRow(apiHelp);
    static_cast<QVBoxLayout *>(bricklink->layout())->insertWidget(1, apiGroup);
...
void SettingsDialog::accept()
{
    if (m_apiTesting)
        return;
    try {
        Config::inst()->setBrickOwlApiKey(m_brickOwlKey->text().trimmed());
        QJsonObject storeApi;
        const QStringList keys { u"consumerKey"_qs, u"consumerSecret"_qs, u"token"_qs, u"tokenSecret"_qs };
        for (size_t i = 0; i < m_storeApiFields.size(); ++i)
            storeApi.insert(keys.at(qsizetype(i)), m_storeApiFields[i]->text().trimmed());
        Config::inst()->setBrickLinkStoreApiCredentials(QString::fromUtf8(QJsonDocument(storeApi).toJson(QJsonDocument::Compact)));
    } catch (const Exception &) {
        QMessageBox::warning(this, tr("Credentials"), tr("Could not save API credentials. Check your system credential storage. Settings remain open."));
        return;
    }
    save();
    QDialog::accept();
...
void SettingsDialog::load()
{
    ...
    m_brickOwlKey->setText(Config::inst()->brickOwlApiKey());
    const auto storeApi = QJsonDocument::fromJson(Config::inst()->brickLinkStoreApiCredentials().toUtf8()).object();
    const QStringList apiKeys { u"consumerKey"_qs, u"consumerSecret"_qs, u"token"_qs, u"tokenSecret"_qs };
    for (size_t i = 0; i < m_storeApiFields.size(); ++i)
        m_storeApiFields[i]->setText(storeApi.value(apiKeys.at(qsizetype(i))).toString());
