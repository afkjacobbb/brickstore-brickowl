...
#if defined(BRICKOWL_PROTOTYPE)
static const char * const application = "BrickStoreBrickOwlPrototype";
#elif defined(BS_MOBILE) && !(defined(Q_OS_IOS) || defined(Q_OS_ANDROID))
    ...
    : QSettings(QString::fromLatin1(organization), QString::fromLatin1(application))
{
    ...
    try {
        m_brickOwlApiKey = QString::fromUtf8(CredentialsManager::load(u"BrickStore"_qs, u"BrickOwl-API-Key"_qs));
        m_brickLinkStoreApiCredentials = QString::fromUtf8(CredentialsManager::load(u"BrickStore"_qs, u"BrickLink-Store-API"_qs));
    } catch (const Exception &) {
        qWarning("Could not load marketplace API credentials");
    }
...
void Config::setBrickOwlApiKey(const QString &key)
{
    if (key == m_brickOwlApiKey)
        return;
    CredentialsManager::save(u"BrickStore"_qs, u"BrickOwl-API-Key"_qs, key.toUtf8());
    m_brickOwlApiKey = key;
}

void Config::setBrickLinkStoreApiCredentials(const QString &json)
{
    if (json == m_brickLinkStoreApiCredentials)
        return;
    CredentialsManager::save(u"BrickStore"_qs, u"BrickLink-Store-API"_qs, json.toUtf8());
    m_brickLinkStoreApiCredentials = json;
}
